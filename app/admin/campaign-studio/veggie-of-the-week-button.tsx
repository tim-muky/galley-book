...
import { buildVeggieBrief, type VeggieWeek } from "@/lib/marketing/veggie-calendar";

export function VeggieOfTheWeekButton({
  week,
  veggie,
}: {
  week: number;
  ...
  const label = locale === "de" ? veggie.de : veggie.en;
  ...
  async function onClick() {
    ...
    const res = await fetch("/api/admin/campaign-studio/runs", {
      ...
      body: JSON.stringify(buildVeggieBrief(week, veggie, locale)),
    });
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        {submitting
          ? `Generating ${label}…`
          : `🥬 Veggie of the Week · ${label} (KW ${week})`}
      </button>
