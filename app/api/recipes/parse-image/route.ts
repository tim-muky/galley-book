...
// Keep parity with COMMON_RULES in lib/recipe-prompts.ts: image imports must
// preserve the source language of the recipe just like URL and text imports do.
const LANGUAGE_RULE = `- Output language: keep name, description, ingredient names, ingredient group labels, and step instructions in the SAME LANGUAGE as the text shown in the image(s) — do NOT translate to English (or any other language). If the recipe is written in German, it stays in German; if it's in Italian, it stays in Italian. Type, season, cuisine, and main_ingredients stay in lowercase English (they're tag values, not display text). Users translate on demand via the in-app Translate button.`;
