...
import { postPhotosToTikTok, postVideoToTikTok, TikTokApiError } from "@/lib/marketing/tiktok";
...
export async function POST(
  ...
  const action: "photo" | "video" = body?.action === "video" ? "video" : "photo";
  ...
  const { data: dist } = await service
    ...
    .select(
      "id, carousel_paths, caption_de, caption_en, post_title, video_path, tiktok_status, tiktok_post_id",
    )
  ...
  // ---- Video (reel) → TikTok's native format -------------------------------
  if (action === "video") {
    if (!dist.video_path) {
      return NextResponse.json(
        { error: "Render the reel first (Reel section), then post it to TikTok" },
        { status: 400 },
      );
    }
    const videoUrl = `${base}/api/public/campaign-reel/${run.published_galley_id}`;
    try {
      const { publishId, status, privacy } = await postVideoToTikTok({ videoUrl, caption });
      await service
        .from("galley_distributions")
      ...
      logger.info("campaign_studio.tiktok.reel_succeeded", { runId: id, publishId, status, privacy });
      ...
      await service
        ...
        .eq("id", dist.id);
      logger.error("campaign_studio.tiktok.reel_failed", { runId: id, message });
